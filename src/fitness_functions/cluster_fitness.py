...
def clusterDistanceFitness(array, height, width, desiredClusterSize):
    ...
    for i in range(numClusters):
        if(clusterMembership[i] >= desiredClusterSize):
            distance = math.hypot(clusterCenters[i][0]-centerH, clusterCenters[i][1]-centerW)
...
def defaultClusterDistanceFitness(timestepBoards):
    ...
    for board in timestepBoards:
        ...
        height = boardShape[0]
        width = boardShape[1]
        fitnessList.append(clusterDistanceFitness(stepBoard, height, width, defaultDesiredClusterSize))
